      ...
      <td>
        <div class="retirada-group">
          <input
            ...
            placeholder="0"
            data-id="${i.id}"
            data-estoque="${i.quantidadeEstoque}"
          />
          <button class="btn-baixar" data-id="${i.id}">
            Baixar
          </button>
        </div>
      </td>
      <td>
        <button class="btn-excluir" data-id="${i.id}">
          Excluir
        </button>
      </td>
    </tr>
...
async function baixarEstoque(id) {
  ...
  if (!validarRetirada(estoqueAtual, quantidadeRetirada)) {
    alert(
      'Quantidade inválida: não pode ser zero, negativa ou maior que o estoque.'
    );
    return;
  }
...
document.addEventListener('DOMContentLoaded', init);function validarRetirada(estoqueAtual, quantidadeRetirada) {
  if (quantidadeRetirada <= 0) return false;
  if (quantidadeRetirada > estoqueAtual) return false;
  return true;
}
